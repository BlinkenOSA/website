...
import {fetchCurrentInternsList} from "@/utils/api/fetchInterns";
import InternCard from "@/components/Cards/InternCard";
import Spacer from "@/components/Spacer/Spacer";

export const getServerSideProps = (async () => {
    const [url, params] = fetchCurrentInternsList()
...
const InternsPage = ({initialData}) => {
    ...
    return (
        <>
            <Head>
                <title>Blinken OSA Archivum | {t('current_interns__title')}</title>
            </Head>
            <div className={style.Page}>
                <Container>
                    <SimplePageHeader title={t('current_interns__title')} menu={'academics'} breadCrumb={'Academics'} />
                    <Row>
                        <Col xs={12}>
                            <Row>
